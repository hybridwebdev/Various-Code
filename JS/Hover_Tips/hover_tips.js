jQuery(document).ready(function($) {
	...
	$("span.highlight_text").live({ // this bit here handles the tooltips. It takes the text (which is hidden) from the field instructions
		// and populates it into the tooltips. 
		
	    mouseenter: function () {
		
	  		var text = $(this).parent().clone()    //clone the element
	        .children('label') //select all the children
		...
		if((text=='') || (text==' ')) {
			...
			var text = $(this).parent().find('span.sub-field-instructions').text();
					
		}
	
		
			var turget = $(this).parent().parent();
		    $(turget).popover({
		        trigger: 'manual',
		        placement: 'top',
		        content: function() {
